...
This directory contains the configuration for a professional-grade development container optimized for C++20 development.

## Architecture

### Custom Docker Image

We use a **custom multi-stage Dockerfile** instead of a base image + install script approach for several key benefits:
...
2. **dev-tools**: Build essentials, compilers, debugging tools
3. **cpp-libs**: C++ libraries and testing frameworks
4. **dev-utils**: GitHub CLI, Python tools, vcpkg
5. **final**: User environment with oh-my-zsh
...
**Note**: First build takes 5-10 minutes as it downloads and compiles everything.
Subsequent rebuilds use Docker layer cache and are much faster.
